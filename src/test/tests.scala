...
object Example:
  ...
  object TestPath:
    given (using Tactic[PathError]): Decoder[TestPath] = Navigable.decode[TestPath](_)
    given pathCreator: PathCreator[TestPath, Forbidden, Drive] = TestPath(_, _)
    given Show[TestPath] = _.render
    def parse(text: Text)(using Tactic[PathError]): TestPath = text.decodeAs[TestPath]
  ...
  object TestLink:
    ...
    def parse(text: Text)(using Tactic[PathError]): TestLink = text.decodeAs[TestLink]
