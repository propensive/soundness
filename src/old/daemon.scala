...
inline def cli(using cli: Invocation): Invocation = cli

case class Invocation(args: List[Text], env: Map[Text, Text], script: File, stdin: LazyList[Bytes],
...
trait App:
  def main(using Invocation, Environment): ExitStatus
...
trait Daemon()(using Log) extends App:
  ...
  def server(script: Text, fifo: Text, serverPid: Int, watchPid: Int)
            (using Monitor)
            : Unit =
    val socket: Path = fifo.decodeAs[Unix.Path]
    ...
    case class CliClient(pid: Pid, spawnId: Int, scriptFile: Maybe[File] = Unset,
      ...
      def pwd: Directory throws EnvironmentError = env.pwd[Path].directory(Expect)
      ...
      def spawn(): Task[Unit] = Task(t"spawn"):
        ...
        try
          ...
          val commandLine = Invocation(args, instanceEnv, scriptFile.or(sys.exit(1)),
      ...
      case (map, line) =>
        line.content.cut(t"\t").to(List) match
          ...
          case t"RUNDIR" :: As[Int](pid) :: dir :: _ =>
            safely(dir.decodeAs[Unix.Path]).option.fold(map): dir =>
              map.updated(pid, map(pid).copy(runDir = dir.directory(Expect)))
          
          case t"SCRIPT" :: As[Int](pid) :: scriptDir :: script :: _ =>
            safely(t"$scriptDir/$script".decodeAs[Unix.Path]).option.map(_.file(Expect)).fold(map): file =>
