...
case class Dag[T] private(edgeMap: Map[T, Set[T]] = Map()):
  ...
  def sources: Set[T] = edgeMap.collect:
    case (k, v) if v.isEmpty => k
  .to(Set)
  
  def edges: Set[(T, T)] = edgeMap.to(Set).flatMap:
    (k, vs) => vs.map(k -> _)

  def dot: String = edges.map:
    (k, v) => s""""$k" -> "$v""""
  .mkString("digraph {\n  ", ";\n  ", ";\n}")
  ...
  def flatMap[S](fn: T => Dag[S]): Dag[S] = Dag:
    edgeMap.flatMap:
      (k, v) => fn(k).edgeMap.map:
        (k2, v2) => k2 -> (v2 ++ v.flatMap(fn(_).keys))
  .reduction
