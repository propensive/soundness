/*
    Escritoire, version 0.4.0. Copyright 2018-23 Jon Pretty, Propensive OÜ.
...
extension [T](maybe: Maybe[T]) def otherwise(that: T): T =
  if maybe == Unset then that else maybe.asInstanceOf[T]

extension (at: AnsiText)
  def padding(n: Int, direction: Direction = Ltr): AnsiText = direction match
    case Ltr => if at.length < n then at+AnsiText(t" "*(n - at.length)) else at
    case Rtl => if at.length < n then AnsiText(t" "*(n - at.length))+at else at
...
case class Table[T](cols: Column[T]*):
  ...
  def tabulate(data: Seq[T], maxWidth: Int, delimitRows: DelimitRows = DelimitRows.RuleIfMultiline)
    ...
    val freeWidth: Int = maxWidth - cols.filter(_.width != Unset).map(_.width.otherwise(0)).sum - style.cost(cols.length)
      ...
      if row >= cells.length then LazyList()
        ...
        then
          val text = columns.indices.map: col =>
            ...
            val ext = extent(content.plain, columns(col).width.otherwise(0), offsets(col))
            val slice = content.slice(offsets(col), ext.left)
            offsets(col) = ext.right
            val width = columns(col).width.otherwise(0)
    ...
    def rule(left: Char, separator: Char, right: Char, bar: Char): AnsiText =
      columns.map { col => ansi"$bar"*(col.width.otherwise(0) + 2) }.join(ansi"$left", ansi"$separator", ansi"$right")
