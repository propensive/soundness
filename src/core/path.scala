...
sealed trait Path[+R <: Root]:
  def parent: Path[R]
  def ancestor(ascent: Int): Path[R]
  def absolute[R2 >: R <: Root](pwd: Absolute[R2]): Absolute[R2]
  def /(filename: String): Path[R]
...
case class Absolute[+R <: Root](root: R, path: Vector[String]) extends Path[R]:
  ...
  def parent: Absolute[R] = path match
    case init :+ last => Absolute[R](root, init)
    case _            => throw RootBoundaryExceeded(root)

  def ancestor(ascent: Int): Absolute[R] = if path.length == 0 then this else parent.ancestor(ascent - 1)
  
  def absolute[R2 >: R <: Root](pwd: Absolute[R2]): Absolute[R2] raises RootBoundaryExceeded = this
  
  def /(filename: String): Absolute[R] = filename match
    case ".."     => path match
  ...
  def ++(relative: Relative): Absolute[R] =
...
case class Relative(ascent: Int, path: Vector[String]) extends Path[Nothing]:
  ...
  def self: Boolean = path.isEmpty

  def parent: Relative = path match
    case init :+ last => Relative(ascent, init)
    case empty        => Relative(ascent + 1, Vector())

  def ancestor(ascent: Int): Relative = if ascent == 0 then this else parent.ancestor(ascent - 1)
  ...
  def /(filename: String): Relative = filename match
    case ".."     => path match
      case init :+ last => Relative(ascent, init)
      case empty        => Relative(ascent + 1, Vector())
    case "."      => this
  ...
  def ++(relative: Relative): Relative =
