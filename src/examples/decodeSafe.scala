...
object DecoderSafe extends Derivation[DecoderSafe]:
  ...
  def join[T](ctx: CaseClass[DecoderSafe, T]): DecoderSafe[T] = value =>
    ...
    ctx.constructEither { param => param.typeclass.decode(values(param.label)) }.left.map(_.reduce(_+"\n"+_))
  ...
  private def parse(value: String): (String, Map[String, String]) =
    ...
    def parts(value: String, idx: Int = 0, depth: Int = 0, collected: List[String] = List("")): List[String] =
      ...
      if (idx == value.length) collected else value(idx) match
        case '(' =>
          parts(value, idx + 1, depth + 1, plus('('))
        case ')' =>
          if (depth == 1) plus(')') else parts(value, idx + 1, depth - 1, plus(')'))
        case ',' =>
    ...
    (name, parts(value.substring(end + 1, value.length - 1)).map(keyValue).to(Map))
