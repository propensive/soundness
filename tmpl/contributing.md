...
${TITLE} is developed using [Fury](https://github.com/propensive/fury), which may be installed by running the
...
The simplest way to compile ${TITLE} is to run,
```
fury
```
...
It&rsquo;s a good idea to [discuss](https://riot.im/app/#/room/#propensive.${ID}:matrix.org) potential changes
...
If a response is not forthcoming in the [Riot chatroom](https://riot.im/app/#/room/#propensive.${ID}:matrix.org),
...
issue before work starts. If you have commit rights to the `propensive/${ID}` repository, please only push to
...
rejected on grounds of formatting, except in extreme cases. ${TITLE} does not use automatic code-formatting
...
development environment for all ${TITLE}&rsquo;s users. Propensive O&Uuml; reserves the right to censure
