...
object Tests extends Suite(m"Stenography Tests"):
  def run(): Unit =
    ...
    . assert(_ == "collection.immutable.List[Int]")
    ...
    test(m"Show match type"):
      Syntax.name[[scrutinee] =>> scrutinee match
        case String => Char
        case Int    => Boolean]
    . assert(_ == "[scrutinee] =>> scrutinee match { case String => Char; case Int => Boolean }")

    test(m"Show match type with parametric case"):
      Syntax.name[[scrutinee] =>> scrutinee match
        case List[item] => item]
    . assert(_ == "[scrutinee] =>> scrutinee match { case List[item] => item }")
    ...
    . assert(_ == "turbulence.Streamable")
    ...
    . assert(_ == "[field >: String <: AnyRef] => (x: field) -> Int")
    ...
    . assert(_ == "Addable { type Q = Int; type Other = String }")
    ...
    . assert(_ == "Addable by Int { type Q = String }")
    ...
    . assert(_ == "(Addable by Int) => String")
    ...
    test(m"Leave a refinement which bounds, rather than aliases, its member"):
      Syntax.name[Addable { type Operand <: Int }]
    . assert(_ == "Addable { type Operand = ? <: Int }")
    ...
    . assert(_ == "Addable { type Hkt = [Bar] =>> Option[Bar] }")
    ...
    . assert(_ == "Addable { type Hkt = [Bar <: String] =>> Option[Bar] }")
    ...
    . assert(_ == "Int is Addable by Double => String is Checkable against String")
    ...
    . assert(_ == "(addable: Int is Addable by Double) ?=> String is Checkable against String")

    test(m"Tuple"):
      Syntax.name[(Int, String, Exception)]
    . assert(_ == "(Int, String, Exception)")

    test(m"Named tuple"):
      Syntax.name[(x: Int, y: String, z: Exception)]
    . assert(_ == "(x: Int, y: String, z: Exception)")
    ...
    . assert(_ == "(e: Enumeration) => e.Value")
