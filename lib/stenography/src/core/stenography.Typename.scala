...
object Typename:
  ...
  def decode(text: Text): Typename =
    def entity(start: Ordinal, end: Ordinal, isType: Boolean, parent: Optional[Typename]): Typename =
    ...
    def recur(position: Ordinal, start: Ordinal, isType: Boolean, typename: Optional[Typename]): Typename =
      text.at(position) match
        case Unset            => entity(start, position - 1, isType, typename)
        case char@('.' | ':') =>
          recur(position + 1, position + 1, char == ':', entity(start, position - 1, isType, typename))
        case char  =>
