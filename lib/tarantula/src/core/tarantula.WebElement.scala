...
object WebElement:
  ...
  given encodable: (WebElement is Encodable in Json) =
...
// and a capability-classed `WebElement` constructed there is fresh at each construction, so a
...
case class WebElement(elementId: Text)
