...
//
// JZlib splits the decompressor into mutually-referencing `ZStream`, `Inflate`, `InfBlocks`,
// `InfCodes` and `InfTree` objects; under separation checking such a cyclic ownership graph is
// inexpressible, so the port flattens them into this single `caps.Mutable` state machine. Each
// original object survives as a contiguous block of fields and methods (`blocksMode`/`codesMode`
// are the former per-object `mode` fields), and the output buffer is threaded through the call
// chain as an exclusive parameter rather than stored.
private[pneumatic] final class Inflater(nowrap: Boolean) extends InflateEngine:
  ...
  :   Int =
    ...
    p = 0
    i = n
    while { c(blens(bindex + p)) += 1; p += 1; i -= 1; i != 0 } do ()
        ...
        while k <= g && !overflow do
          ...
          while a != 0 && !overflow do
            ...
            while k > w + l && !overflow do
              ...
              // allocate new table
              if hn + z > Many then overflow = true // overflow of Many
              else
                q = hn
                u(h) = q
                hn += z
                ...
                if h != 0 then
                  ...
                  j = i >>> (w - l)
                  r(2) = q - u(h - 1) - j
                  System.arraycopy(r, 0, hufts, (u(h - 1) + j)*3, 3) // connect to last table
                else
                  huftTable = q // first table is returned result
            ...
            if !overflow then
              ...
              while j < z do
                System.arraycopy(r, 0, hufts, (q + j)*3, 3)
                j += f
  ...
  private update def inflateTreesBits(): Int =
    initWorkArea(19)
    hn = 0
    huftTable = tb
    huftBits = bb
    var result = huftBuild(0, 19, 19, Inflater.noExtra, Inflater.noExtra)
    tb = huftTable
    bb = huftBits

    if result == ZDataError then msg = "oversubscribed dynamic bit lengths tree"
    else if result == ZBufError || bb == 0 then
      msg = "incomplete dynamic bit lengths tree"
      result = ZDataError

    result

  private update def inflateTreesDynamic(nl: Int, nd: Int): Int =
    // build literal/length tree
    initWorkArea(288)
    hn = 0
    huftTable = tli
    huftBits = bl
    var result = huftBuild(0, nl, 257, cplens, cplext)
    tli = huftTable
    bl = huftBits

    if result != ZOk || bl == 0 then
      if result == ZDataError then msg = "oversubscribed literal/length tree"
      ...
      huftTable = tdi
      huftBits = bd
      result = huftBuild(nl, nd, 0, cpdist, cpdext)
      tdi = huftTable
      bd = huftBits

      if result != ZOk || (bd == 0 && nl > 257) then
        if result == ZDataError then msg = "oversubscribed distance tree"
  ...
  private update def codesProc(r0: Int, target: scala.Array[Byte]^): Int =
    ...
    def leave(status: Int, b: Int, k: Int, p: Int, n: Int, q: Int): Int =
      bitb = b; bitk = k
      availIn = n; totalIn += p - nextInIndex; nextInIndex = p
      write = q
      inflateFlush(status, target)
      ...
      codesMode match
        case Start => // x: set up for Len
          if m >= 258 && n >= 10 then
            ...
            if r != ZOk then codesMode = if r == ZStreamEnd then Wash else Badcode

          if codesMode == Start then
            codesNeed = lbits
            tree = ltree
        ...
        case Len => // i: get length/literal/eob next
          ...
          else if (e & 32) != 0 then codesMode = Wash // end of block
          else
            codesMode = Badcode // invalid code
        ...
        case Copy => // o: copying bytes in window, waiting for space
          ...
          while f < 0 do f += windowEnd // modulo window size — "while" handles invalid distances

          var bail = false

          while len != 0 && !bail do
            if m == 0 then
              if q == windowEnd && read != 0 then
                q = 0
                m = if q < read then read - q - 1 else windowEnd - q

              if m == 0 then
                write = q
                r = inflateFlush(r, target)
                q = write
                m = if q < read then read - q - 1 else windowEnd - q
  ...
  :   Int =
    ...
    def leave(status: Int, b: Int, k0: Int, p0: Int, n0: Int, q: Int): Int =
      ...
      val k1 = k0 - (c0 << 3)
      bitb = b; bitk = k1
      availIn = n1; totalIn += p1 - nextInIndex; nextInIndex = p1
      write = q
      status
            ...
            while !distDone do
              ...
              if (e & 16) != 0 then
                ...
                if q >= d then // offset before dest: just copy
                  ...
                  while { r += windowEnd; r < 0 } do () // force pointer in window
  ...
  var blocksMode: Int = Type // current inflate_block mode
  ...
  private var bd: Int = 0
  private var tli: Int = 0 // literal/length tree index within hufts
  private var tdi: Int = 0 // distance tree index within hufts
  ...
  val windowEnd: Int = 1 << wbits // one byte after sliding window
  var read: Int = 0   // window read pointer
  var write: Int = 0  // window write pointer

  private update def blocksReset(): Unit =
    blocksMode = Type
    bitk = 0
    bitb = 0
    read = 0
  ...
  private update def blocksProc(r0: Int, target: scala.Array[Byte]^): Int =
    ...
    var n = 0 // bytes available there
    var q = 0 // output window write pointer
    var m = 0 // bytes to windowEnd of window or read pointer
    ...
    while true do
      blocksMode match
        case Type =>
          ...
          (t >>> 1) match
            case 0 => // stored
              ...
              b >>>= t
              k -= t
              blocksMode = Lens // get length of stored block

            case 1 => // fixed
              codesInit(fixedBl, fixedBd, fixedLtree, 0, fixedDtree, 0)
              b >>>= 3
              k -= 3
              blocksMode = Codes
        ...
        case Lens =>
          ...
          b = 0 // dump bits
          k = 0
          blocksMode = if left != 0 then Stored else if last != 0 then Dry else Type
        ...
        case Stored =>
          ...
          if t > n then t = n
          if t > m then t = m
          System.arraycopy(nextIn, p, window, q, t)
          p += t
          n -= t
          q += t
          m -= t
          left -= t
          if left == 0 then blocksMode = if last != 0 then Dry else Type
        ...
        case Btree =>
          ...
          if t != ZOk then
            r = t
            if r == ZDataError then blocksMode = InfBlocks.Bad
            return leave(r, b, k, p, n, q)
        ...
        case Dtree =>
          ...
          if t != ZOk then
            if t == ZDataError then blocksMode = InfBlocks.Bad
            r = t
            return leave(r, b, k, p, n, q)

          codesInit(bl, bd, hufts, tli, hufts, tdi)
          blocksMode = Codes
        ...
        case Codes =>
          ...
          r = codesProc(r, target)
          if r != ZStreamEnd then return inflateFlush(r, target)
  ...
  private update def inflateFlush(r0: Int, target: scala.Array[Byte]^): Int =
    ...
    // compute number of bytes to copy as far as windowEnd of window
    var n = (if q <= write then write else windowEnd) - q
    if n > availOut then n = availOut
    ...
    if q == windowEnd then
      // wrap pointers
      q = 0
      if write == windowEnd then write = 0
  ...
  private update def run(f0: Int, target: scala.Array[Byte]^): Int =
    ...
    while true do
      mode match
        case Inflater.Head =>
          if wrap == 0 then mode = Inflater.Blocks
          else
            if needBytes == -1 then
              needBytes = 2
              need = 0
            ...
            if (method & 0xf) != ZDeflated then
              ...
              adler = Adler32()
              if (b & PresetDict) == 0 then mode = Inflater.Blocks else mode = Inflater.Dict4

        case Inflater.Dict4 | Inflater.Dict3 | Inflater.Dict2 | Inflater.Dict1 | Inflater.Dict0 =>
          mode = Inflater.Bad
          msg = "preset dictionaries are not supported"
          return ZStreamError

        case Inflater.Blocks =>
          r = blocksProc(r, target)

          if r == ZDataError then mode = Inflater.Bad
          else
            if r == ZOk then r = f
            ...
            if wrap == 0 then mode = Inflater.Done else mode = Inflater.Check4

        case Inflater.Check4 =>
          if availIn == 0 then return r
          r = f
          availIn -= 1
          totalIn += 1
          ...
          nextInIndex += 1
          mode = Inflater.Check3

        case Inflater.Check3 =>
          if availIn == 0 then return r
          r = f
          availIn -= 1
          totalIn += 1
          ...
          nextInIndex += 1
          mode = Inflater.Check2

        case Inflater.Check2 =>
          if availIn == 0 then return r
          r = f
          availIn -= 1
          totalIn += 1
          ...
          nextInIndex += 1
          mode = Inflater.Check1

        case Inflater.Check1 =>
          if availIn == 0 then return r
          r = f
          availIn -= 1
          totalIn += 1
