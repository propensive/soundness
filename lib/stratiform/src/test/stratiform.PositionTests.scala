  ...
  def run(): Unit =
    suite(m"Top-level compounds"):
      ...
      test(m"Locate a top-level compound's keyword"):
        t"greeting hello\n".read[Tel].locateKey(TelPath(List(t"greeting")))
      . assert(_ == at(1, 1, 8))

      test(m"Locate the keyword of the second of two top-level compounds"):
        t"first a\nsecond b\n".read[Tel].locateKey(TelPath(List(t"second")))
      . assert(_ == at(2, 1, 6))
    ...
    suite(m"Nested compounds"):
      test(m"Locate a child compound's keyword"):
        t"person\n  name Alice\n  age 30\n".read[Tel]
        . locateKey(TelPath(List(t"person", t"name")))
      . assert(_ == at(2, 3, 4))

      test(m"Locate the keyword of a sibling child on a later line"):
        t"person\n  name Alice\n  age 30\n".read[Tel]
        . locateKey(TelPath(List(t"person", t"age")))
      . assert(_ == at(3, 3, 3))

      test(m"Locate a grandchild compound's keyword"):
        t"a\n  b\n    c hello\n".read[Tel].locateKey(TelPath(List(t"a", t"b", t"c")))
      . assert(_ == at(3, 5, 1))
    ...
    suite(m"Column tracks indentation"):
      test(m"A top-level keyword is at column 1"):
        t"root\n  child value\n".read[Tel].locateKey(TelPath(List(t"root"))).let(_.column)
      . assert(_ == 1)

      test(m"A one-level-deep keyword is at column 3"):
        t"root\n  child value\n".read[Tel]
        . locateKey(TelPath(List(t"root", t"child"))).let(_.column)
      . assert(_ == 3)

      test(m"A one-level-deep keyword is on the second line"):
        t"root\n  child value\n".read[Tel]
        . locateKey(TelPath(List(t"root", t"child"))).let(_.line)
      . assert(_ == 2)
    ...
    suite(m"Value extents"):
      ...
      test(m"Locate the value of an indented child"):
        t"item\n  unit-cost  banana\n".read[Tel]
        . locate(TelPath(List(t"item", t"unit-cost")))
      . assert(_ == at(2, 14, 6))

      test(m"A value spans from the first atom to the last"):
        t"range 1 to 10\n".read[Tel].locate(TelPath(List(t"range")))
      . assert(_ == at(1, 7, 7))

      test(m"A hard-space gap falls inside the value's span"):
        t"note a  b\n".read[Tel].locate(TelPath(List(t"note")))
      . assert(_ == at(1, 6, 4))
      ...
      test(m"locateKey at the root is Unset"):
        t"greeting hello\n".read[Tel].locateKey(TelPath(Nil))
      . assert(_ == Unset)
      ...
      test(m"Every path in a deep, wide document resolves to its own value"):
        ...
        List
         ( tel.locate(TelPath(List(t"a"))),
           tel.locate(TelPath(List(t"b", t"c"))),
           tel.locate(TelPath(List(t"b", t"d", t"e"))),
           tel.locate(TelPath(List(t"b", t"d", t"f"))),
           tel.locate(TelPath(List(t"b", t"g"))),
           tel.locate(TelPath(List(t"h"))) )
      . assert(_ == List
                     ( at(1, 3, 1),
                       at(3, 5, 1),
                       at(5, 7, 1),
