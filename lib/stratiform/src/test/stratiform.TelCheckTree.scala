...
object TelCheckTree:
  ...
  private def ofTabulation(tabulation: Tel.Tabulation): CheckTree =
    CheckTree.Struct
      ( t"Tabulation",
        List
         ( t"marker_offsets" -> CheckTree.Sequence(List.of(tabulation.markerOffsets.toList.map(CheckTree.Num(_)))),
           t"headings"       -> CheckTree.Sequence(List.of(tabulation.headings.toList.map(h => CheckTree.Str(h)))) ) )
  ...
  private def ofIArray[value](items: IArray[value])(f: value => CheckTree): CheckTree =
    CheckTree.Sequence(List.of(items.toList.map(f)))
