...
object TelPointer:
  ...
  val Empty: TelPointer = TelPointer(IArray.empty)

  def of(keywords: Text*): TelPointer =
    TelPointer(IArray.from(keywords.map(Step(_, Unset))))

case class TelPointer(steps: IArray[TelPointer.Step]):
  def /(keyword: Text): TelPointer =
    TelPointer(steps :+ TelPointer.Step(keyword, Unset))

  def /(keyword: Text, index: Int): TelPointer =
    TelPointer(steps :+ TelPointer.Step(keyword, index))
