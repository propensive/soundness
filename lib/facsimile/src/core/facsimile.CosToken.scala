...
// generic `Keyword`s for the parser to interpret, which is what lets the same lexer serve
// both file bodies and content streams.
private[facsimile] enum CosToken:
  case Integral(value: Long)
  case Real(value: Double)
  case Name(text: Text)
  case Chars(bytes: Data)
  case ArrayStart, ArrayEnd
  case DictStart, DictEnd
