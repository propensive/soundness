...
private[facsimile] object ContentWriter:
  ...
  :   Unit =
    ...
    def nums(values: Double*): Text = values.map(num).join(" ")
    ...
    operator match
      ...
      case SetIntent(intent)        => name(intent); out(" ri\n")
      case SetFlatness(tolerance)   => out(t"${num(tolerance)} i\n")
      case SetParameters(gs)        => name(gs); out(" gs\n")
      ...
      case Stroke                            => out("S\n")
      case CloseStroke                       => out("s\n")
      case Fill(FillRule.NonZero)            => out("f\n")
      case Fill(FillRule.EvenOdd)            => out("f*\n")
      case FillStroke(FillRule.NonZero)      => out("B\n")
      case FillStroke(FillRule.EvenOdd)      => out("B*\n")
      case CloseFillStroke(FillRule.NonZero) => out("b\n")
      case CloseFillStroke(FillRule.EvenOdd) => out("b*\n")
      case EndPath                           => out("n\n")
      case Clip(FillRule.NonZero)            => out("W\n")
      case Clip(FillRule.EvenOdd)            => out("W*\n")

      case BeginText                => out("BT\n")
      case EndText                  => out("ET\n")
      ...
      case ShowText(data)           => string(data); out(" Tj\n")
      case NextLineShow(data)       => string(data); out(" '\n")
      ...
      case StrokeSpace(name0)       => name(name0); out(" CS\n")
      case FillSpace(name0)         => name(name0); out(" cs\n")
      ...
      case Draw(name0)              => name(name0); out(" Do\n")
      case Shade(name0)             => name(name0); out(" sh\n")
      ...
      case MarkPoint(tag, Unset)    => name(tag); out(" MP\n")
      case BeginMarked(tag, Unset)  => name(tag); out(" BMC\n")
      ...
      case EndMarked                => out("EMC\n")
      case BeginCompatibility       => out("BX\n")
      case EndCompatibility         => out("EX\n")
