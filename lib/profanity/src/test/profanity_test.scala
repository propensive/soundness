  ...
  def run(): Unit =
    ...
    suite(m"Keyboard decoding"):
      ...
      suite(m"Interrupt POSIX numbering"):
        ...
        test(m"SIGQUIT is 3") (Interrupt.Quit.id)  .assert(_ == 3)
        test(m"SIGKILL is 9") (Interrupt.Kill.id)  .assert(_ == 9)
        test(m"SIGTERM is 15")(Interrupt.Term.id)  .assert(_ == 15)
        test(m"SIGCHLD is 17")(Interrupt.Chld.id)  .assert(_ == 17)
        test(m"SIGCONT is 18")(Interrupt.Cont.id)  .assert(_ == 18)
        test(m"SIGSTOP is 19")(Interrupt.Stop.id)  .assert(_ == 19)
