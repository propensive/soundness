...
abstract class Worker(frame: Codepoint, parent: Monitor^, probate: Probate^) extends Monitor:
  ...
  def stack: Text =
    val ref = // The `(x: Text)` ascriptions widen singleton-bounded values (case-2 pure-value box).
      name.lay((frame.text: Text).s)(name => (name: Text).s+"@".s+(frame.text: Text).s)

    parent match
      case root: Root         => ((root.supervisor.name: Text).s+"://".s+ref).tt
      case submonitor: Worker => ((submonitor.stack: Text).s+"//".s+ref).tt
      case _                  => ref.tt
