...
package httpBackends:
  ...
  inline given wasiHttp: Http.Backend = new Http.Backend:
    ...
    :   Http.Response =
      ...
      val fieldsHandle = Foreign["fields", Wit].constructor.call[Wasm.Handle of "fields"]()
      ...
      val requestHandle =
        outgoingRequest.constructor(fieldsHandle).call[Wasm.Handle of "outgoing-request"]()
      ...
      request.`set-method`(Wasm.Case["method"](method.show.lower)).call[Unit]()
      request.`set-scheme`(Wasm.Case["scheme"](scheme.lower)).call[Unit]()
      ...
      val futureHandle =
        outgoingHandler.handle(requestHandle, Unset)
        . call[Wasm.Handle of "future-incoming-response"]()
      ...
      val pollableHandle = future.subscribe.call[Wasm.Handle of "pollable"]()
      ...
      // is end-of-stream.
      val bodyHandle = response.consume.call[Wasm.Handle of "incoming-body"]()
      val incomingBody: Foreign of "incoming-body" from Wit = bodyHandle
      val streamHandle = incomingBody.selectDynamic("stream").call[Wasm.Handle of "input-stream"]()
...
object WasiHttpServer:
  ...
  :   Unit =
    ...
    val method: Http.Method =
      unsafely(incoming.method.call[Wasm.Case of "method"]().name.upper.as[Http.Method])

    val target: Text = incoming.`path-with-query`.call[Optional[Text]]().or(t"/")

    val headersHandle = incoming.headers.call[Wasm.Handle of "fields"]()
    ...
    val bodyHandle = incoming.consume.call[Wasm.Handle of "incoming-body"]()
    val incomingBody: Foreign of "incoming-body" from Wit = bodyHandle
    val streamHandle = incomingBody.selectDynamic("stream").call[Wasm.Handle of "input-stream"]()
    ...
    val responseHandle =
      outgoingResponse.constructor(fieldsHandle).call[Wasm.Handle of "outgoing-response"]()
    ...
    val outBodyHandle = outgoing.body.call[Wasm.Handle of "outgoing-body"]()
    ...
    val outparamHandle =
      new Wasm.Handle(responseOut).asInstanceOf[Wasm.Handle of "response-outparam"]
