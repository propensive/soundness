          ...
          case other =>
            ...
            if symbol.exists then sci.List(Ref(symbol).asExprOf[Status]) else
              report.errorAndAbort
               ( s"exoskeleton: every status an execute block returns must be a singleton object "
                 + s"extending Status, but the result type includes ${other.show}, which is not "
                 + s"a singleton. Declare each status as a top-level object; if this type is "
                 + "Status itself, the union of statuses has been widened (soundness#1811)." )
        else if repr <:< terminationType then sci.Nil
        else
          report.errorAndAbort
           ( s"exoskeleton: an execute block must return a Termination, such as an Exit or a "
             + s"Status, but the result type includes ${repr.show}, which is neither." )
