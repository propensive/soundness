  ...
  :   line =
    ...
    val parts = tiles.stdlib.zip(widths.stdlib).zipWithIndex.map:
      ...
      case ((t, w), _) =>
        ...
        val cell =
          if base.s.length >= 1 then base.s.charAt(0).toString else (" ": String)

        val filler =
          if base.s.length >= 2 then base.s.charAt(1).toString else (" ": String)

        val padding = if w > 1 then filler.repeat(w - 1).nn else ("": String)
