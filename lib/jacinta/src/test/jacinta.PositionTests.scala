  ...
  def run(): Unit =
    suite(m"Single-line root primitives"):
      test(m"Locate a root number"):
        val json = Json.parseTracked("42")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 2))

      test(m"Locate a root string"):
        val json = Json.parseTracked(""""hello"""")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 7))

      test(m"Locate a root boolean"):
        val json = Json.parseTracked("true")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 4))

      test(m"Locate a root null"):
        val json = Json.parseTracked("null")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 4))
    ...
    suite(m"Single-line objects"):
      test(m"Locate the root object"):
        val json = Json.parseTracked("""{"a":1,"b":2}""")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 13))

      test(m"Locate the value at key 'a'"):
        val json = Json.parseTracked("""{"a":1,"b":2}""")
        json.locate(JsonPointer()("a"))
      . assert(_ == at(1, 6, 1))

      test(m"Locate the value at key 'b'"):
        val json = Json.parseTracked("""{"a":1,"b":2}""")
        json.locate(JsonPointer()("b"))
      . assert(_ == at(1, 12, 1))

      test(m"Locate the key 'a' itself"):
        val json = Json.parseTracked("""{"a":1,"b":2}""")
        json.locateKey(JsonPointer()("a"))
      . assert(_ == at(1, 2, 3))

      test(m"Locate the key 'b' itself"):
        val json = Json.parseTracked("""{"a":1,"b":2}""")
        json.locateKey(JsonPointer()("b"))
      . assert(_ == at(1, 8, 3))

      test(m"Missing key returns Unset"):
        val json = Json.parseTracked("""{"a":1}""")
        json.locate(JsonPointer()("missing"))
      . assert(_ == Unset)

    suite(m"Single-line arrays"):
      test(m"Locate the root array"):
        val json = Json.parseTracked("[10,20,30]")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 10))
      ...
      test(m"Out-of-bounds index returns Unset"):
        val json = Json.parseTracked("[10,20]")
        json.locate(JsonPointer()(Ter))
      . assert(_ == Unset)

      test(m"Empty array locates only the root"):
        val json = Json.parseTracked("[]")
        json.locate(JsonPointer())
      . assert(_ == at(1, 1, 2))

    suite(m"Nested structures"):
      test(m"Locate a key in a nested object"):
        val json = Json.parseTracked("""{"a":{"b":42}}""")
        json.locate(JsonPointer()("a")("b"))
      . assert(_ == at(1, 11, 2))

      test(m"Locate an element of a nested array"):
        val json = Json.parseTracked("""{"xs":[1,2,3]}""")
        json.locate(JsonPointer()("xs")(Sec))
      . assert(_ == at(1, 10, 1))

      test(m"Locate an object inside an array"):
        val json = Json.parseTracked("""[{"id":1},{"id":2}]""")
        json.locate(JsonPointer()(Sec)("id"))
      . assert(_ == at(1, 17, 1))
    ...
    suite(m"Multi-line input"):
      test(m"Second-line key is on line 2"):
        ...
        json.locate(JsonPointer()("a")).let(_.line)
      . assert(_ == 2)

      test(m"Second-line value column is past the indent"):
        val source = "{\n  \"a\": 42\n}"
        val json = Json.parseTracked(source)
        json.locate(JsonPointer()("a")).let(_.column)
      . assert(_ == 8)
