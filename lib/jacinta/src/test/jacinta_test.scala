...
case class InvalidState(name: String) extends Exception(s"Not a valid state: $name")
...
case class Marked(@memo("the count") n: Int)
case class Renamed(@name[Json]("first_name") firstName: Text, @name("yob") year: Int)
derives CanEqual
...
enum Status derives CanEqual:
  @name[Json]("ok") case Active(since: Int)
  ...
  def run(): Unit =
    suite(m"Parsing tests"):
      ...
      test(m"Parse a string"):
        val s = "\"string\"".read[Json]
        s.as[Text]
      . assert(_ == "string")

      test(m"Parse true"):
        "true".read[Json].as[Boolean]
      . assert(identity)

      test(m"Parse false"):
        "false".read[Json].as[Boolean]
      . assert(!_)

      test(m"Parse float"):
        "3.1415".read[Json].as[Float]
      . assert(_ == 3.1415f)

      test(m"Parse double"):
        "3.1415926".read[Json].as[Double]
      . assert(_ == 3.1415926)

      test(m"Parse a long"):
        "1234567890123".read[Json].as[Long]
      ...
      test(m"Parse null as Unit"):
        "null".read[Json].as[Unit]
      . assert(_ == ())

      test(m"Parse an empty array"):
        "[]".read[Json].as[List[Int]]
      . assert(_ == Nil)

      test(m"Parse an array of numbers"):
        "[1, 2, 3]".read[Json].as[List[Int]]
      . assert(_ == List(1, 2, 3))
    ...
    suite(m"Native-rendering coverage"):
      ...
      test(m"jacinta's types inspect natively"):
        Inspectable.fallbacks
         ( "foo".in[Json].inspect, 42.in[Json].inspect, Json.Ast("foo").inspect,
    ...
    suite(m"Serialization"):
      ...
      test(m"Serialize a nested case class"):
        NamedOuter("hello", Inner(7)).in[Json].show
      . assert(_ == """{"name":"hello","inner":{"n":7}}""")

    suite(m"Misc tests"):
      test(m"Serialize to Json"):
        Foo(1, "two").in[Json]
      . assert(_ == Json.make(x = 1.in[Json], y = "two".in[Json]))
      ...
      test(m"Access an absent Optional dynamically"):
        ...
        """{"y": 1}""".read[Json].missing.as[Optional[Int]]
      . assert(_ == Unset)

      test(m"Apply a case-class default when the field is omitted"):
        """{"name": "Eve"}""".read[Json].as[WithDefault]
      . assert(_ == WithDefault("Eve", 18))

      test(m"An explicit value overrides a case-class default"):
        """{"name": "Eve", "age": 30}""".read[Json].as[WithDefault]
      . assert(_ == WithDefault("Eve", 30))

    suite(m"Generic derivation tests"):
      val paul =
        test(m"Serialize a simple case class"):
          Person("Paul", 81).in[Json].show
        .check(_ == """{"name":"Paul","age":81}""")

      val john = """{"name": "John", "age": 40}"""
      val george = """{"name": "George", "age": 58}"""
      val ringo = """{"name": "Ringo", "age": 82}"""
      ...
      val paulCoproduct = test(m"Serialize a coproduct"):
        ...
        paul.in[Json].show
      .check(_ == """{"person":{"name":"Paul","age":81},"kind":"Bassist"}""")
      ...
      .check(_ == """{"members":[{"person":{"name":"Paul","age":81},"kind":"Bassist"},{"person":{"name":"Ringo","age":82},"kind":"Drummer"},{"person":{"name":"John","age":40},"kind":"Guitarist"},{"person":{"name":"George","age":58},"kind":"Guitarist"}]}""")
      ...
      test(m"Update a JSON object dynamically"):
        import dynamicAccess.dynamicJson
        val john = """{"name": "John", "age": 40}""".as[Json]
    ...
    suite(m"Recursive derivation tests"):
      ...
      val treeJson =
        """{"value":"root","children":[{"value":"a","children":[]},{"value":"b","children":[{"value":"c","children":[]}]}]}"""
      ...
      test(m"Round-trip a directly-recursive type via Optional"):
        val value = TreeOpt("a", TreeOpt("b", Unset))
        value.in[Json].show.read[Json].as[TreeOpt]
      . assert(_ == TreeOpt("a", TreeOpt("b", Unset)))

      test(m"Round-trip recursion through a Map value"):
        val forest = Forest(Map("x" -> Tree("x", Nil), "y" -> tree))
        forest.in[Json].show.read[Json].as[Forest]
      . assert(_ == Forest(Map("x" -> Tree("x", Nil), "y" -> tree)))
    ...
    suite(m"Json construction"):
      test(m"Json.make with one field"):
        Json.make(name = "Anna".in[Json]).show
      . assert(_ == """{"name":"Anna"}""")

      test(m"Json.make with multiple fields"):
        Json.make(a = 1.in[Json], b = "two".in[Json], c = true.in[Json]).show
      . assert(_ == """{"a":1,"b":"two","c":true}""")

      test(m"Nested Json.make"):
        Json.make(outer = Json.make(inner = 1.in[Json])).show
      . assert(_ == """{"outer":{"inner":1}}""")
    ...
    suite(m"Json field/index access"):
      val obj = """{"name": "Alice", "age": 30}""".read[Json]
      val arr = "[10, 20, 30]".read[Json]
      ...
      test(m"apply on absent field returns absent Json"):
        obj("missing").as[Optional[Int]]
      . assert(_ == Unset)

      test(m"Access nested fields via apply"):
        val nested = """{"a": {"b": {"c": 42}}}""".read[Json]
        nested("a")("b")("c").as[Int]
      . assert(_ == 42)

      test(m"Access object then array element"):
        val mixed = """{"items": [1, 2, 3]}""".read[Json]
        mixed("items")(2).as[Int]
      . assert(_ == 3)

      test(m"Dynamic field access"):
        import dynamicAccess.dynamicJson
        val person = """{"name": "Bob"}""".read[Json]
        person.name.as[Text]
      . assert(_ == "Bob")

      test(m"Dynamic indexed access of array field"):
        import dynamicAccess.dynamicJson
        val data = """{"nums": [4, 5, 6]}""".read[Json]
        data.nums(1).as[Int]
      . assert(_ == 5)
    ...
    suite(m"Json updates"):
      ...
      test(m"Set a field to a string"):
        import dynamicAccess.dynamicJson
        val base = """{"x": 1}""".read[Json]
        val updated = base.greeting = "hi"
        updated.show
      . assert(_ == """{"x":1,"greeting":"hi"}""")
    ...
    suite(m"Json.Ast conversions"):
      ...
      test(m"primitive of null is Null"):
        Json.unseal("null".read[Json]).primitive
    ...
    suite(m"Direct parsing tests"):
      ...
      test(m"an inlined Map field stays on the runtime seam"):
        ...
        val counts = Counts(Map("a" -> 1, "b" -> 2), "tag")
        counts.in[Json].show.read[Counts in Json]
      . assert(_ == Counts(Map("a" -> 1, "b" -> 2), "tag"))
      ...
      test(m"Read a Long directly"):
        "1234567890123".read[Long in Json]
      . assert(_ == 1234567890123L)

      test(m"Read a Double directly"):
        "3.1415926".read[Double in Json]
      . assert(_ == 3.1415926)

      test(m"Read a Boolean directly"):
        "true".read[Boolean in Json]
      . assert(identity)

      test(m"Read a Text directly"):
        "\"hello world\"".read[Text in Json]
      ...
      test(m"Direct number coercion matches the AST path"):
        "3.9".read[Long in Json]
      . assert(_ == 3L)

      test(m"Read a Json value through the direct path"):
        """{"a": 1}""".read[Json in Json].as[Json]
      . assert: json =>
          import dynamicAccess.dynamicJson
        ...
        given DirectPoint is Json.Parsable = Json.Parsable(Morphology.Any): reader =>
          ...
          while
            reader.key().lay(false): key =>
              if key == "x" then x = reader.int()
              else if key == "y" then y = reader.int()
    ...
    suite(m"Derived direct parsing tests"):
      ...
      test(m"A full record round-trips equally on both paths"):
        val json = """{"guitarists": [{"name": "John", "age": 40}, {"name": "George", "age":
      ...
      test(m"A top-level collection of an opted-in element reads directly"):
        """[{"name": "Amy", "age": 50}, {"name": "Bea", "age": 60}]"""
      ...
      test(m"A mistyped directly-parsed field raises Parse.Error"):
        ...
        capture[Parse.Error]("""{"name": "Amy", "age": "x"}""".read[Person in Json])
      . assert(_.issue match
          case Json.Ast.Issue.ExpectedNumber('"') => true
          case _                                  => false)

      test(m"A whole Data block reads directly, without stream plumbing"):
        """{"name": "Amy", "age": 50}""".in[Data].read[Person in Json]
      . assert(_ == Person("Amy", 50))

      test(m"An 18-digit-plus integer falls back to the general number path"):
        parity[Person]("""{"name": "Old", "age": 99999999999999999999}""")
      . assert(identity)

      test(m"A leading zero raises the same issue on both paths"):
        val direct = capture[Parse.Error]("""{"name": "Amy", "age": 0123}""".read[Person in Json])
        val ast = capture[Parse.Error]("""{"name": "Amy", "age": 0123}""".read[Json])
        (direct.issue, ast.issue)
      . assert: (directIssue, astIssue) =>
          directIssue == astIssue

      test(m"A negative integer reads on the fast path"):
        """{"name": "Amy", "age": -3}""".read[Person in Json]
      . assert(_ == Person("Amy", -3))

      test(m"Malformed content inside a skipped field raises Parse.Error"):
        capture[Parse.Error]("""{"name": "Amy", "junk": {"x": }, "age": 50}""".read[Person in Json])
      . assert(_.issue match
    ...
    suite(m"Staged direct parsing tests"):
      ...
      test(m"A staged parser accepts reordered and unknown fields"):
        """{"extra": [{}], "age": 50, "name": "Amy"}""".read[Person in Json]
      . assert(_ == Person("Amy", 50))
      ...
      test(m"A staged parser raises Absent for missing required fields"):
        capture[Json.Error]("""{"age": 50}""".read[Person in Json]).reason
      . assert(_ == Json.Error.Reason.Absent)

      test(m"A staged parser honors @name renames"):
        agree[Renamed]("""{"first_name": "Jon", "yob": 1983}""")
      . assert(identity)
      ...
      test(m"A staged parser reads collection and option fields"):
        val json = """{"guitarists": [{"name": "John", "age": 40}], "drummer":
      ...
      test(m"A staged parser handles recursive types"):
        agree[Tree]("""{"value": "a", "children": [{"value": "b", "children": []}]}""")
      . assert(identity)
      ...
      test(m"A staged parser is last-wins for duplicate keys"):
        agree[Person]("""{"name": "Amy", "name": "Bea", "age": 50}""")
      . assert(identity)
      ...
      test(m"A staged sum parser dispatches on the discriminator field"):
        ...
        ( """{"radius": 2.5, "kind": "Circle"}""".read[Shape in Json],
          """{"kind": "Square", "side": 3.0}""".read[Shape in Json] )
      ...
      test(m"A staged sum parser agrees with the AST path"):
        ...
        given Shape.Square is Json.Parsable = Json.Parsable.staged
        given Shape is Json.Parsable = Json.Parsable.staged
        agree[Shape]("""{"radius": 2.5, "kind": "Circle"}""")
      . assert(identity)

      test(m"A staged sum parser raises Absent for a missing discriminator"):
        given Shape is Json.Parsable = Json.Parsable.staged
        capture[Json.Error]("""{"radius": 2.5}""".read[Shape in Json]).reason
      . assert(_ == Json.Error.Reason.Absent)

      test(m"A staged sum parser raises Variant.Error for an unknown tag"):
        given Shape is Json.Parsable = Json.Parsable.staged
        capture[Variant.Error]("""{"radius": 2.5, "kind": "Blob"}""".read[Shape in Json])
        . inputLabel
      . assert(_ == "Blob")
      ...
      test(m"A staged sum parser honors variant renames"):
        ...
        """{"since": 3, "kind": "ok"}""".read[Status in Json]
    ...
    suite(m"Json error handling"):
      ...
      test(m"Asking for a boolean when JSON is null raises Absent or NotType"):
        capture[Json.Error]("null".read[Json].as[Boolean])
      . assert(_.reason match
    ...
    suite(m"Discriminator strategies"):
      test(m"Discriminate by 'kind' (default in this file)"):
        ...
        s.in[Json].show
      . assert(_ == """{"radius":1.0,"kind":"Circle"}""")
    ...
    suite(m"Time encodables/decodables"):
      ...
      . assert(_ == "1700000000000")

      test(m"Decode an Instant from a Long"):
        "1700000000000".read[Json].as[Instant over Unix].long
      ...
      test(m"Decode a Duration from a Long of milliseconds"):
        "5000".read[Json].as[Duration].value
      . assert(_ == 5.0)
    ...
    suite(m"JsonSchema tests"):
      ...
      test(m"JsonSchema Format decodes from kebab case"):
        "date-time".as[JsonSchema.Format]
    ...
    suite(m"j\"\" interpolator"):
      ...
      test(m"Object rest splice"):
        ...
        j"""{"a": 1, $rest}"""
      . assert(_ == """{"a": 1, "b": 2, "c": 3}""".read[Json])

    suite(m"j\"\" extractor"):
      test(m"Extract single value"):
        val scrutinee: Json = """{"a": 42}""".read[Json]
        scrutinee match
          case j"""{"a": $a}""" => a.as[Int]
          case _                => -1
      . assert(_ == 42)
      ...
      test(m"Match exact array"):
        val scrutinee: Json = """[1, 2, 3]""".read[Json]
        scrutinee match
      ...
      test(m"Mismatched array length is rejected"):
        val scrutinee: Json = """[1, 2]""".read[Json]
        scrutinee match
          case j"""[$a, $b, $c]""" => true
          case _                   => false
      . assert(!_)
      ...
      test(m"Head/tail destructure"):
        val scrutinee: Json = """[1, 2, 3, 4]""".read[Json]
        scrutinee match
      ...
      test(m"Capture top-level Json"):
        val scrutinee: Json = """{"k": 1}""".read[Json]

        (scrutinee: @unchecked) match
          case j"$x" => x
      . assert(_ == """{"k": 1}""".read[Json])
    ...
    suite(m"Specific per-path overrides"):
      val firm = Firm(Worker("ann", 30), Worker("bob", 40))
      ...
      test(m"Without a Specific, all fields use default encoders"):
        firm.in[Json].show
      . assert(_ == """{"boss":{"name":"ann","age":30},"deputy":{"name":"bob","age":40}}""")
      ...
      test(m"a collection element is overridden via a local given + re-derive"):
        ...
        Crew(Worker("al", 30), List(Worker(t"bo", 40), Worker(t"cy", 50))).in[Json].show
      . assert(_ == """{"lead":{"name":"al","age":30},"members":["bo","cy"]}""")
