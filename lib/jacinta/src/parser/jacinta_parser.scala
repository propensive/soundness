...
extension (json: JsonAst.type)
  ...
  def parse(input: Iterator[Data]): JsonAst raises ParseError =
    JsonAst(JsonParser.parse(input))

  def parse(input: Iterator[Data], holes: Boolean): JsonAst raises ParseError =
    JsonAst(JsonParser.parse(input, holes))

given parserAggregable: Tactic[ParseError] => JsonAst is Aggregable by Data =
  source => JsonAst.parse(source.iterator)
