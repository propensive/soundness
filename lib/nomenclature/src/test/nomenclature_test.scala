...
object Tests extends Suite(m"Nomenclature tests"):
  def run(): Unit =
    ...
    test(m"Create a successful new name with inference"):
      val name: Name[Id] = Name[Id]("hello!")
      name
    . assert(_ == "hello!")

    test(m"Name must not start with 0"):
      capture[Name.Error](Name[Id]("0hello!")).message.show
    . assert(_ == "the name 0hello! is not valid because it must not start with 0")

    test(m"Name must end with !"):
      capture[Name.Error](Name[Id]("hello!9")).message.show
    . assert(_ == "the name hello!9 is not valid because it must end with !")

    test(m"Name must not contain ."):
      capture[Name.Error](Name[Id]("hello.world!")).message.show
    . assert(_ == "the name hello.world! is not valid because it must not contain .")

    test(m"Name must not equal ."):
      capture[Name.Error](Name[Id2](".")).message.show
    . assert(_ == "the name . is not valid because it must not equal .")
    ...
    test(m"Covariance probe: a wider plane intersection is a subtype"):
      val wide: Name[Id2 & EndsO] = "hello".asInstanceOf[Name[Id2 & EndsO]]
    ...
    test(m"Name is required"):
      capture[Name.Error](Name[Required]("")).message.show
    . assert(_ == """the name “” is not valid because it must not be empty""")

    test(m"A valid CSS class name is accepted"):
      Name[CssClass]("main-nav")
    . assert(_ == "main-nav")

    test(m"A CSS class name may not start with a digit"):
      capture[Name.Error](Name[CssClass]("1col")).message.show
    . assert(_ == "the name 1col is not valid because it must be a valid CSS identifier")

    test(m"A valid DOM id is accepted"):
      Name[DomId]("main-content")
    . assert(_ == "main-content")

    test(m"A DOM id may not contain whitespace"):
      capture[Name.Error](Name[DomId]("a b")).message.show
    . assert(_ == "the name a b is not valid because it must be a valid DOM id")
    ...
    test(m"Decode a friendly name to a moniker"):
      given (Vocabulary over Session) = Vocabulary(adjectives, animals)
      "brilliant-leopard".as[Moniker over Session].ordinal
    . assert(_ == 10351)

    test(m"Round-trip a moniker through its name"):
      given (Vocabulary over Session) = Vocabulary(adjectives, animals)
      "brilliant-leopard".as[Moniker over Session].encode
    . assert(_ == "brilliant-leopard")
    ...
    . assert(_ == ("brilliant-leopard", "capybara-tart"))

    test(m"An out-of-range number cannot be encoded"):
      given (Vocabulary over Session) = Vocabulary(adjectives, animals)
      capture[Moniker.Error](Moniker[Session](100000).encode).message.show
    . assert(_ == "the moniker is not valid because the number 100000 is outside the representable range")

    test(m"A malformed name cannot be decoded"):
      given (Vocabulary over Session) = Vocabulary(adjectives, animals)
      capture[Moniker.Error]("justoneword".as[Moniker over Session]).message.show
    . assert(_ == "the moniker is not valid because justoneword is not of the form <adjective>-<animal>")

    test(m"An unknown word cannot be decoded"):
      given (Vocabulary over Session) = Vocabulary(adjectives, animals)
      capture[Moniker.Error]("notathing-leopard".as[Moniker over Session]).message.show
    . assert(_ == "the moniker is not valid because the word notathing does not appear in the vocabulary")
    ...
    suite(m"Native-rendering coverage"):
      test(m"nomenclature's types inspect natively"):
        given (Vocabulary over Session) = Vocabulary(adjectives, animals)
        Inspectable.fallbacks(Name[EndsO]("foo").inspect, Moniker[Session](10351).inspect)
      . assert(_ == Nil)

      test(m"a name is distinguishable from the text it is"):
        Name[EndsO]("foo").inspect
      . assert(_ == "n\"foo\"")
    ...
    suite(m"MustStart"):
      ...
      test(m"a name without the required prefix is rejected"):
        capture[Name.Error](Name[Prefixed]("ray")).message.show
      . assert(_ == "the name ray is not valid because it must start with x-")

      test(m"the prefix must be at the start, not merely present"):
        capture[Name.Error](Name[Prefixed]("ray-x-")).message.show
      . assert(_ == "the name ray-x- is not valid because it must start with x-")
    ...
    suite(m"MustContain"):
      ...
      test(m"a name containing the required text is accepted"):
        Name[Containing]("main-nav")
      . assert(_ == "main-nav")

      test(m"a name not containing the required text is rejected"):
        capture[Name.Error](Name[Containing]("mainnav")).message.show
      . assert(_ == "the name mainnav is not valid because it must contain -")
    ...
    suite(m"MustNotEnd"):
      ...
      test(m"a name not ending with the forbidden text is accepted"):
        Name[NotEndingZ]("quartz-a")
      . assert(_ == "quartz-a")

      test(m"a name ending with the forbidden text is rejected"):
        capture[Name.Error](Name[NotEndingZ]("quartz")).message.show
      . assert(_ == "the name quartz is not valid because it must not end with z")

      test(m"the forbidden text is permitted away from the end"):
        Name[NotEndingZ]("zebra")
      . assert(_ == "zebra")
    ...
    suite(m"MustMatch"):
      ...
      test(m"a name not matching the pattern is rejected"):
        capture[Name.Error](Name[Lowercase]("abc1")).message.show
      . assert(_ == "the name abc1 is not valid because it must match [a-z]+")

      test(m"the empty name does not match a one-or-more pattern"):
        capture[Name.Error](Name[Lowercase]("")).message.show
      . assert(_ == """the name “” is not valid because it must match [a-z]+""")
    ...
    suite(m"MustMatch anchoring"):
      ...
      test(m"the pattern must match the whole name, not a substring"):
        capture[Name.Error](Name[Repeated]("baa")).message.show
      . assert(_ == "the name baa is not valid because it must match a+")

      test(m"a trailing substring match is also not enough"):
        capture[Name.Error](Name[Repeated]("aab")).message.show
      . assert(_ == "the name aab is not valid because it must match a+")
    ...
    suite(m"MustNotMatch"):
      ...
      test(m"a name matching the forbidden pattern is rejected"):
        capture[Name.Error](Name[NotDigits]("123")).message.show
      . assert(_ == "the name 123 is not valid because it must not match [0-9]+")
