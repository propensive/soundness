...
object Pcm:
  ...
  extends Openable:
    ...
    :   result =
      ...
      val rate = flags.glean { case PcmFlag.Rate(hertz) => hertz }.or(44100)
      val bits = flags.glean { case PcmFlag.Bits(bits) => bits }.or(16)
      val chunk = flags.glean { case PcmFlag.Chunk(bytes) => bytes }.or(65536)
