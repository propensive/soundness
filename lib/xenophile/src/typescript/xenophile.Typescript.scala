...
// foreign types `Typescript.Dialect` reads from `.ts` definition files. No runtime representation is
...
object Typescript:
  ...
  // TypescriptDialect → Typescript.Dialect
  ...
  object Parser:
    ...
    def parse(source: Text): List[TypescriptDeclaration] raises Typescript.Error =
    ...
    private class Cursor(tokens: List[Token]):
      ...
      def declarations(): List[TypescriptDeclaration] raises Typescript.Error =
      ...
      :   TypescriptDeclaration raises Typescript.Error =
...
trait Typescript extends Ecosystem:
  type Grammar = Typescript.Dialect.type
