...
// A loaded native library, paired with the C signatures `CHeader.Dialect` read from
...
object ForeignLibrary:
  ...
  // Maps a C type (as canonicalised by `CHeader.Dialect`) to an FFM memory layout.
