...
object Cell:
  ...
  private def slice(cell: Cell, row: Int): Text =
    cell.lines.at(row.z).let(_.text).or(spaces(cell.width).text)
    ...
    case _ =>
      ...
      val lines = Sequence.from:
        (0 until height).map: row =>
          Writing(List.of(framed.map { cell => cell.lines.at((row).z).vouch.text }).join)
      ...
      (0 until height).map: row =>
        ...
        val scripts =
          if row < superscript.height then
            val text = superscript.lines.at((row).z).vouch.text
            val pad = spaces(right - superscript.width).text
            t"$text$pad"
          else if row >= middle then
            val text = subscript.lines.at((row - middle).z).vouch.text
  ...
  def radical(inner: Cell): Cell =
    if inner.height > 1 then tallRadical(inner) else
      ...
      val body = Writing(t"$Radical${inner.lines.at((0).z).vouch.text}")
  ...
  private def tallRadical(inner: Cell): Cell =
    ...
    val body = Sequence.from:
      (0 until inner.height).map: row =>
        ...
        Writing(t"$foot$Stem${inner.lines.at((row).z).vouch.text}")
  ...
  def root(base: Cell, index: Cell): Cell =
    ...
    val lines = Sequence.from:
      (0 until radicand.height).map: row =>
        val prefix = if row < index.height then index.lines.at((row).z).vouch.text else spaces(index.width).text
        Writing(t"$prefix${radicand.lines.at((row).z).vouch.text}")
