...
case class Mstack(contents: List[Mathml], attributes: List[(Text, Text)] = Nil) extends Elementary:
...
case class Msgroup(contents: List[Mathml], attributes: List[(Text, Text)] = Nil) extends Elementary:
  def label: Text = t"msgroup"

object Msrow:
  def apply(children: Mathml*): Msrow = Msrow(children.to(List))

case class Msrow(contents: List[Mathml], attributes: List[(Text, Text)] = Nil) extends Elementary:
...
case class Mscarry(contents: List[Mathml], attributes: List[(Text, Text)] = Nil) extends Elementary:
