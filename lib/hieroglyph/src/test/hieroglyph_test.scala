...
object Tests extends Suite(m"Hieroglyph tests"):
  def run(): Unit =
    ...
    suite(m"Roundtrip decoding"):
      ...
      for chunk <- 1 to 25 do
        test(m"Decode Japanese text in chunks of size $chunk"):
          import textSanitizers.skipSanitizer
          charDecoders.utf8Decoder.decoded(japaneseData.stdlib.grouped(chunk).map(Array.frozen(_)).to(Progression)).join
        . assert(_ == japanese)
      ...
      test(m"Ensure that decoding is finished"):
        ...
        capture[CharDecodeError](charDecoders.utf8Decoder.decoded(Array.frozen(t"café".in[Data].stdlib.dropRight(1))))
    ...
    suite(m"Compile-time tests"):
      ...
      . assert(_ == List(t"[↯SN-541] the encoding ABCDEF was not available"))
