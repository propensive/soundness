...
object internal:
  ...
  object Text:
    ...
    def apply(chars: IArray[Char]): Text = make(String(chars.asInstanceOf[scala.Array[Char]]))
    def apply(bytes: IArray[Byte]): Text = make(String(bytes.asInstanceOf[scala.Array[Byte]], "ASCII"))
