...
object Tests extends Suite(m"Plutocrat tests"):
  def run(): Unit =
    ...
    suite(m"ISINs"):
      ...
      . assert(_ == List(t"[↯SN-515.3] the ISIN number is not valid because it had length 11, but it should be 12 characters long"))

      test(m"Compiletime ISIN bad country code"):
        demilitarize(isin"5500BH4HKS30").map(_.message)
      . assert(_ == List(t"[↯SN-515.2] the ISIN number is not valid because its country code 55 was not valid"))

      test(m"Compiletime ISIN invalid character"):
        demilitarize(isin"US00Bx4HKS30").map(_.message)
      . assert(_ == List(t"[↯SN-515.1] the ISIN number is not valid because the character x at position 5 is not a digit or uppercase letter"))
