...
package spinners:
  ...
  private val line: Spinner = Spinner.each("-\\|/", 130, Ascii)
  private val quadrant: Spinner = Spinner.each("◴◷◶◵", 120, Unicode, line)
  private val arc: Spinner = Spinner.each("◜◠◝◞◡◟", 100, Unicode, line)
  private val pulse: Spinner = Spinner.each("█▓▒░▒▓", 100, Unicode, line)
  private val circles: Spinner = Spinner.each("◌○◎◉●◉◎○", 120, Unicode, line)
  private val halves: Spinner = Spinner.each("◐◓◑◒", 120, Unicode, line)
  private val hourglass: Spinner = Spinner.each("⧖⧗", 500, Unicode, line)

  given lineSpinner: Gauging => Fraction is Gaugeable = line.gaugeable

  given crossStarSpinner: Gauging => Fraction is Gaugeable =
    Spinner.each("+x*", 120, Ascii).gaugeable

  given dqpbSpinner: Gauging => Fraction is Gaugeable =
    Spinner.each("dqpb", 100, Ascii).gaugeable

  given bounceSpinner: Gauging => Fraction is Gaugeable =
    Spinner.each(".oO°Oo.", 120, Ascii).gaugeable

  given balloonSpinner: Gauging => Fraction is Gaugeable =
    Spinner.each(" .oO@* ", 140, Ascii).gaugeable
  ...
  given starSpinner: Gauging => Fraction is Gaugeable =
    Spinner.each("✶✸✹✺✹✷", 70, Unicode, line).gaugeable
