...
trait Functor[functor[_]] extends Findable:
  ...
  extension [value](value: functor[value])
    def map[value2](lambda: value => value2): functor[value2] = apply(value)(lambda)

  def apply[value, value2](value: functor[value])(lambda: value => value2): functor[value2]

object Functor:
  // `List` is opaque, so its `map` is invisible to Mercator's derivation macro; this explicit
  // instance routes through the `.scala` escape hatch and the stdlib `map`.
