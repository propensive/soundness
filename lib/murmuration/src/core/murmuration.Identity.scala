...
object Identity:
  // `List` is opaque, so its `apply` is invisible to Mercator's derivation macro; this explicit
  // instance supplies it directly.
  given list: Identity[List]:
    def point[value](value: value): List[value] = List(value)

  given set: Identity[Set]:
