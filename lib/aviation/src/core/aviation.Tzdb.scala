...
object Tzdb:
  ...
  def parse(name: Text, lines: Chain[Text]): List[Tzdb.Entry] logs Tzdb.Event raises Tzdb.Error =
    ...
    // The zone currently being accumulated, as a one-or-no-element list to concatenate with.
    def pending(zone: Optional[Tzdb.Entry.Zone]): List[Tzdb.Entry] = zone.lay(Nil)(List(_))
    ...
    def recur
      ( lineNo:  Int,
        ...
        zone:    Optional[Tzdb.Entry.Zone]   = Unset )
    :   List[Tzdb.Entry] =
