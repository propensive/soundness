...
trait protointernal extends caps.Pure:
    ...
    def principal(using Quotes): UnitRef =
      ...
      dimensionType.absolve match case '[type dimension <: Dimension; dimension] =>
        Expr.summon[Principal[dimension, ?]].absolve match
          case None =>
            val dimensionName =
              dimensionality.quantityName.map: name => s"the physical quantity $name"
              . getOrElse(s"the same quantity")
        ...
        Expr.summon[Ratio[from & to, ?]].absolve match
          case None =>
            if retry then ratio(to, from, -power, false)
              ...
              val dimensionName = quantityName.map(s"the physical quantity "+_).getOrElse:
  ...
  private def incompatibleTypeText(left: UnitsMap, right: UnitsMap)(using Quotes): String =
    (left.dimensionality.quantityName, right.dimensionality.quantityName) match
      case (Some(leftName), Some(rightName)) =>
        s"quantitative: the left operand represents $leftName, but the right " +
          s"operand represents $rightName; these are incompatible physical quantities"
      ...
      case None =>
        ' {
            Multiplicable[multiplicand, multiplier, Double]: (left, right) =>
              ${quantitative.internal.multiply('left, 'right).asExprOf[Double]}
          }
      ...
      case Some('[type result <: Measure; result]) =>
        ' {
            Divisible[dividend, divisor, Quantity[result]]: (left, right) =>
              ${quantitative.internal.divide('left, 'right).asExprOf[Quantity[result]]}
          }

      case None =>
        ' {
            Divisible[dividend, divisor, Double]: (left, right) =>
              ${quantitative.internal.divide('left, 'right).asExprOf[Double]}
          }
      ...
      case Some('[type result <: Measure; result]) =>
        ' {
            Divisible[Double, divisor, Quantity[result]]: (left, right) =>
              ${quantitative.internal.divide('{Quantity(left)}, 'right).asExprOf[Quantity[result]]}
          }

      case None =>
        ' {
            Divisible[Double, divisor, Double]: (left, right) =>
              ${quantitative.internal.divide('{Quantity(left)}, 'right).asExprOf[Double]}
          }
      ...
      case Some('[type result <: Measure; result]) =>
        ' {
            Divisible[Int, divisor, Quantity[result]]:
              (left, right) =>
                $ {
                    quantitative.internal.divide('{Quantity(left.toDouble)}, 'right)
      ...
      case None =>
        ' {
            Divisible[Int, divisor, Double]: (left, right) =>
              ${quantitative.internal.divide('{Quantity(left.toDouble)}, 'right).asExprOf[Double]}
          }
    ...
    units.repr.map(_.asType).absolve match case Some('[type measure <: Measure; measure]) =>
      ' {
          Subtractable[quantity, quantity2, Quantity[measure]]:
            (left, right) =>
              ${quantitative.internal.sub('left, 'right).asExprOf[Quantity[measure]]}
        }
    ...
    units.repr.map(_.asType).absolve match case Some('[type result <: Measure; result]) =>
      ' {
          Addable[quantity, quantity2, Quantity[result]]: (left, right) =>
            ${quantitative.internal.add('left, 'right).asExprOf[Quantity[result]]}
        }
    ...
    units.repr.map(_.asType).absolve match case Some('[type result <: Measure; result]) =>
      ' {
          Checkable[quantity, quantity2]: (left, right) =>
            ${quantitative.internal.check('left, 'right)}
        }
