...
object Bytecode:
  ...
  object Opcode:
    ...
    given Opcode is Showable =
      ...
      case Invokevirtual(cls, name) => val cls2 = StackTrace.rewrite(cls.s)
                                       val name2 = StackTrace.rewrite(name.s, true)
                                       t"invoke·virtual $cls2 ⌗ $name2"
      case Invokespecial(cls, name) => val cls2 = StackTrace.rewrite(cls.s)
                                       val name2 = StackTrace.rewrite(name.s, true)
                                       t"invoke·special $cls2 . $name2"
      case Invokestatic(cls, name)  => val cls2 = StackTrace.rewrite(cls.s)
                                       val name2 = StackTrace.rewrite(name.s, true)
                                       t"invoke·static $cls2 . $name2"
      case Invokeinterface(cls, name, _) => val cls2 = StackTrace.rewrite(cls.s)
                                            val name2 = StackTrace.rewrite(name.s, true)
                                            t"invoke·interface $cls2 ⌗ $name2"
      case Invokedynamic(name)      => val name2 = StackTrace.rewrite(name.s, true)
