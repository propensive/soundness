...
trait Creature extends Dynamic:
  def selectDynamic(name: String): Text = ""

object Tests extends Suite(m"Harlequin Tests"):
  def run(): Unit =
    val snippet = "val xs = List(1, 2, 3)"
    ...
    suite(m"Native-rendering coverage"):
      test(m"harlequin's types inspect natively"):
        Inspectable.fallbacks
         ( Token("val", Accent.Keyword).inspect,
           Token("xs", Accent.Term, role = Role.Binding).inspect )
      . assert(_ == Nil)
    ...
    test(m"a val name is a term binding"):
      tagOf("val alpha = 1", "alpha")
    .assert(_ == ("term", "binding"))

    test(m"a term usage is a term reference"):
      tagOf("val beta = gamma", "gamma")
    .assert(_ == ("term", "usage"))

    test(m"a type usage is a typal usage"):
      tagOf("val n: List = xs", "List")
    .assert(_ == ("typal", "usage"))

    test(m"a tuple-pattern val binds each name as a term"):
      val source = "val (aa, bb) = pair"
      (tagOf(source, "aa"), tagOf(source, "bb"))
    .assert(_ == (("term", "binding"), ("term", "binding")))

    test(m"a case-clause pattern binder is a term binding"):
      tagOf("val qq = nums.map { case xx => xx }", "xx")
    .assert(_ == ("term", "binding"))

    test(m"a for-comprehension binder is a term binding"):
      val source = "val qq = for cc <- items yield cc"
      (tagOf(source, "cc"), tagOf(source, "items"))
    .assert(_ == (("term", "binding"), ("term", "usage")))

    test(m"a defined type parameter is a typal binding"):
      tagOf("def gg[TT](vv: TT) = vv", "TT")
    .assert(_ == ("typal", "binding"))

    test(m"a value parameter is a term binding"):
      tagOf("def gg[TT](vv: TT) = vv", "vv")
    .assert(_ == ("term", "binding"))
    ...
    test(m"typechecked highlighting resolves the type of a val"):
      ...
      typeOf(Scala.highlight(snippet).lines.to[List].stdlib.flatMap(_.stdlib).to(List), "xs").or(t"")
    .assert { rendered => rendered.subsumes("List") && rendered.subsumes("Int") }
    ...
    test(m"typechecked highlighting reports diagnostics for ill-typed code"):
      ...
      Scala.highlight("val n: Int = \"oops\"").diagnostics.size
    .assert(_ > 0)
    ...
    test(m"a type application completes in-scope types"):
      ...
      val source = "val x = collection.mutable.Map[Li"
      Scala.highlight(source, caret = source.length.z).completions.lay(Nil)(_.items.map(_.name))
    .assert(_.has("List"))
    ...
    test(m"a Dynamic receiver completes through its Completable companion"):
      ...
      val source =
        "val creature: harlequin.Creature = new harlequin.Creature {}\nval x = creature.ha"

      Scala.highlight(source, caret = source.length.z).completions.lay(Nil)(_.items.map(_.name))
    .assert(_.has("habitat"))
    ...
    suite(m"Elaborations"):
      ...
      val snippet =
        "def pick[element](x: element)(using Ordering[element]): element = x\nval chosen = pick(9)"

      test(m"an inferred type argument is recorded on the callee token"):
        elaborationsOf(snippet, "pick").flatMap(_.typeArguments.stdlib).map(_.qualified)
      . assert(_.exists(_.subsumes("Int")))

      test(m"a synthesized using argument names the resolved given"):
        elaborationsOf(snippet, "pick").flatMap(_.givenArguments.stdlib).map(_.name)
      . assert(_.exists(_.subsumes("Ordering")))

      test(m"explicit arguments are never recorded"):
        val explicit =
          "def pick[element](x: element)(using Ordering[element]): element = x\n"
          + "val chosen = pick[Int](9)(using Ordering.Int)"

        elaborationsOf(explicit, "pick")
      . assert(_.isEmpty)
    ...
    suite(m"Keyword completions"):
      ...
      test(m"transparent offers inline and trait"):
        keywordsAt("transparent ")
      . assert { words => words.has("inline") && words.has("trait") }

      test(m"transparent inline unambiguously offers definitions"):
        keywordsAt("transparent inline ")
      ...
      test(m"a call's argument list offers expressions, not definitions"):
        val words = keywordsAt("foo(")
        (words.has("new"), words.has("val"))
      . assert(_ == (true, false))
      ...
      test(m"a fresh binding position suppresses all completions"):
        Scala.highlight("val ", caret = "val ".size.z).completions.let(_.items.size)
      . assert(_ == 0)
      ...
      test(m"an indented template body after a colon offers definitions"):
        keywordsAt("class Foo:\n  va")
      . assert(_ == List(t"val", t"var"))

      test(m"an operator continues an expression"):
        keywordsAt("val x = 1 + ")
      . assert { words => words.has("new") && !words.has("val") }
