...
case object Directory extends WindowsEntry, UnixEntry:
  // DirectoryHandle → Directory.Handle, DirectoryOpenable → Directory.Openable
  ...
  trait Handle extends caps.ExclusiveCapability:
    type Plane <: Subtree
    type Under <: Platform
  ...
  extends aperture.Openable:
    ...
    type Result = Handle { type Under = filesystem }

    def open[grants <: Grant, result]
      ( value: path, mode: Mode granting grants, flags: List[Nothing] )
      ( block: (((Handle { type Under = filesystem }) & Granting[grants])^) ?=> result )
    :   result =
      ...
      try
        val handle =
          new Handle with Granting[grants]:
