...
object Redraft:
  ...
  def render(diff: Diff[Text], context: Context): Redraft =
    ...
    context match
      ...
      case Context.Minimal =>
        Redraft(minimize(resolved, original, diff.patch(original, (left, _) => left).to(List))*)
  ...
  :   List[Directive] =
    ...
    def valid(candidate: List[Directive]): Boolean =
      ...
      anomalies.nil && Diff(edits*).patch(original, (left, _) => left).to(List) == target
...
case class Redraft(directives: Redraft.Directive*):
  ...
  :   Stream[Text] raises RedraftError =
    ...
    resolve(original, compare).patch(original, (left, _) => left)
