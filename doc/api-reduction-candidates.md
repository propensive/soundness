...
- exported multi-word type-level names: **593**, regenerated from the `soundness_*`
  files at this commit. 355 sit in a prefix family of two or more, across 107 families;
  238 are singletons
- passes three to five removed 63, 8 and 31 respectively, and added three names that had to
  become reachable: `YamlPath` (so `YamlPath.Error` stays exported), `Css.Syntax` (previously
  `@unexported` to dodge a clash with stenography's) and `Svg.Id` (previously re-exported from
  `object internal`)
...
`TraversalOrder`, `TrieMap`, `TripleDes`, `TtfTag`, `TypescriptDialect`, `UdpResponse`,
