...
```scala
HttpServer.listen:
  val name = param(t"name")
  val age = param(t"age")

  Response(t"The name is $name and age is $age")
```
...
HttpServer.listen:
  request match
    case Path(t"/")                  => homePage
    case Path(t"/contact")           => contactUsPage
    case Path(t"/products/$product") => productPage(product)
```
...
HttpServer.listen:
  request match
    case Path(t"/") & RequestHeader.UserAgent(r"Mozilla/.*")                => mozillaHome
    case Path(t"/") & Post() & AcceptEncoding(lang) if lang.contains(t"en") => englishHome
```
...
Telekinesis uses the `Url` type to represent a URI. This will always use either the `http` or `https` URL
...
arguments, and `Text` parameters, like so:
```scala
uri.query(param = t"one", value = t"two", flag = t"three", option = t"four")
```
...
Default given `ToQuery` instances are provided for the primitive types, `Text` and `Int`, and instances will
...
For example,
```scala
case class Address(number: Int, street: Text, city: Text)
case class Person(name: Text, address: Address)

val person = Person(t"Jack", Address(17, t"East Street", t"Birmingham"))
...
`Redirect` takes a single parameter, a representation of a location typically as a `Url` or a `Text`, but
...
[![Scintillate](https://github.com/propensive/scintillate/raw/main/doc/images/128x128.png)](https://github.com/propensive/scintillate/) &nbsp; [![Tarantula](https://github.com/propensive/tarantula/raw/main/doc/images/128x128.png)](https://github.com/propensive/tarantula/) &nbsp;
