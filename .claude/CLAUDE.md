...
- Run all tests: `make test` (builds the umbrella assembly, then `fume run`; needs the `fume` binary on the path — tests and benchmarks have no main class and are only runnable through fume). Selection terms pass through as `make test TESTS='parser*'`.
...
- The attest build runs mill with `--no-daemon` (each invocation is its own short-lived JVM, so concurrent attests/`mill -w` on the same machine never collide, and no `mill shutdown` is needed) and `-j 6` (caps concurrent compilers to bound peak heap; as fast as `-j 12` on a clean build but ~1.5 GB lower). Memory is sized for a 24 GB box: mill heap `-Xmx8g` (`.mill-jvm-opts`), the tests run inside the fume daemon's JVM, which takes no heap flags of its own (set `JAVA_TOOL_OPTIONS` before the daemon starts to size it). Override the job count with `SOUNDNESS_CI_JOBS`.
